class HousedetailController < ApplicationController
    ...
    def houseDirectionAll
        @house = Housedetail.find_by_sql('select district,count(*) as num,avg(unitPrice) as uprice,avg(attention) as anum,avg(square) as snum from housedetail group by district')
        render :json => @house,status:'200 OK'
    end
    def new
    end
